...
function makePmHeaders(user?: string, pass?: string) {
  return {
    ...
    "PM-Metrics":
      "score,siteTotal,sourceTotal,siteIntensity,sourceIntensity,siteIntensityWN,sourceIntensityWN,medianSiteIntensity,percentBetterThanSiteIntensityMedian",
...
// ✅ Only accept months that have write-worthy metrics: score or Site EUI (incl. weather-normalized)
//    Do NOT let medians or totals stop the fallback.
const hasWriteWorthy =
...
if (hasWriteWorthy) {
  picked = { year, month };
  textForParse = text;
  // (Optional) Keep a single name for the picked URL; we'll reuse it below.
  pickedUrl = url;
  results.push({
    propertyId,
    debug: { pickedYear: year, pickedMonth: month, firstWithData: true, note: "score/EUI present" }
  });
  break; // stop; we found a month worth writing
} else {
  results.push({
    propertyId,
    debug: {
      ...
      triedMonth: month,
      firstWithData: false,
      note: "No score or EUI in this month; keep falling back"
    }
  });
  // loop continues to older month
...
const safeUrl = `${PM_BASE}/property/${propertyId}/metrics?year=${qYear}&month=${qMonth}&measurementSystem=EPA`;
...
if (!parsed?.propertyMetrics) {
  results.push({ propertyId, ok: false, status: 200, error: "no propertyMetrics node", url: safeUrl, raw: text });
  continue;
}
	...
	source_eui_wn_kbtu_ft2: num(m.sourceIntensityWN?.value),
     ...
     // Skip if important metrics are missing (median alone isn't useful)
      ...
      if (missingImportantData) {
        results.push({
          propertyId,
          ok: true,
          skipped: "no-score-or-eui",
          url: safeUrl,
          debug_metric_names: Object.keys(m),
          note: "Has median but no score/EUI - likely incomplete month"
        });
        continue;
      }
      ...
      if (upErr) {
        results.push({ propertyId, ok: false, status: 500, error: `upsert failed: ${upErr.message}`, url: safeUrl, row });

      } else {
        let snapshotStatus: "written" | "write-failed" = "written";
        let snapshotError: string | null = null;

        const { error: snapErr } = await supabase
          .from("pm_property_metric_snapshots")
          .upsert(snapshotRow, {
            onConflict: "building_id,metric_as_of_date,snapshot_date",
          });

        if (snapErr) {
          snapshotStatus = "write-failed";
		...
		// Send response after processing all buildings
