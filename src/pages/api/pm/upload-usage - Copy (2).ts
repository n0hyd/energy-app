...
async function ensureMeterInServiceDate(
  ...
  // 2) If it already contains an inServiceDate, skip update
  if (existingXml.includes("<inServiceDate>")) {
    // We could parse and compare dates, but PM typically doesn't complain
    // if the inServiceDate is older than the billing data. We'll leave it.
    log("meter already has inServiceDate, skipping PUT", {
      pmMeterId,
    });
    return;
  }

  // 3) Insert <inServiceDate> before the closing </meter> tag
  const closeIdx = existingXml.lastIndexOf("</meter>");
  if (closeIdx === -1) {
    console.error(
      `Could not find </meter> tag in XML for meter ${pmMeterId}; skipping inServiceDate update.`
    );
    return;
  }
...
// Fetch existing PM consumption periods so we can skip duplicates
...
export default async function handler(
  ...
  try {
    ...
    const { data: pmMeters, error: mErr } = await sb
      .from("meters")
      .select("id, building_id, pm_meter_id")
    ...
    for (const pmMeterId of pmMeterIds) {
      ...
      // Fetch existing PM consumption periods and drop any duplicates
      const existingPeriods = await getExistingConsumptionPeriods(pmMeterId);
      if (existingPeriods.length) {
        const existingKeys = new Set(
          existingPeriods.map((p) => `${p.startDate}|${p.endDate}`)
        );
        const beforeCount = records.length;
        records = records.filter((r) => {
          const key = `${String(r.startDate).slice(0, 10)}|${String(
            r.endDate
          ).slice(0, 10)}`;
