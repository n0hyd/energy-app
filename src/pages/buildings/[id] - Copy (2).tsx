...
function UploadUsageButton({ buildingId }: { buildingId: string }) {
  const [busy, setBusy] = React.useState(false);
  const [out, setOut] = React.useState<string | null>(null);
  const [err, setErr] = React.useState<string | null>(null);
  const [pmUser, setPmUser] = React.useState("");
  const [pmPass, setPmPass] = React.useState("");
  ...
  async function onClick() {
    ...
    if (!pmUser || !pmPass) {
      setErr("Enter your PM username and password.");
      setBusy(false);
      return;
    }

    try {
      const resp = await fetch(`/api/pm/upload-usage?buildingId=${encodeURIComponent(buildingId)}`, {
        method: "POST",
        headers: {
          "x-pm-username": pmUser,
          "x-pm-password": pmPass,
        },
      });
    ...
    <div className="mt-3 rounded-md border border-slate-200 p-3">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:gap-3">
        <div className="flex-1">
          <label className="block text-xs font-medium text-slate-700">PM Username</label>
          <input
            className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
            value={pmUser}
            onChange={(e) => setPmUser(e.target.value)}
            placeholder="bsnrjones-api"
            autoComplete="off"
          />
        </div>
        <div className="flex-1">
          <label className="block text-xs font-medium text-slate-700">PM Password</label>
          <input
            className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
            type="password"
            value={pmPass}
        ...
        >
          {busy ? "Uploading…" : "Upload Usage to PM"}
        </button>
      </div>
...
<div className="mb-6">
  ...
  <div className="mt-2">
    <UploadUsageButton buildingId={building.id} />
  </div>
