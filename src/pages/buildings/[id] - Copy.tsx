...
type Bill = {
  ...
  meter_id: string;
  created_at: string;
  utility_provider: string | null; // <-- add this
};
...
const ACTIVITY_CODES: { code: string; label: string }[] = [
  { code: "K-12 School", label: "K-12 School (score eligible)" },
  { code: "Pre-school/Daycare", label: "Pre-school/Daycare" },
  { code: "Adult Education", label: "Adult Education" },
  { code: "College/University", label: "College/University" },
  { code: "Vocational School", label: "Vocational School" },
  { code: "Other - Education", label: "Other - Education" },
  { code: "Office", label: "Office" },
  { code: "Warehouse/Distribution Center", label: "Warehouse/Distribution Center" },
  { code: "Non-Refrigerated Warehouse", label: "Non-Refrigerated Warehouse" },
  { code: "Refrigerated Warehouse", label: "Refrigerated Warehouse" },
  { code: "Residence Hall/Dormitory", label: "Residence Hall/Dormitory" },
  { code: "Parking", label: "Parking" },
  { code: "Indoor Arena", label: "Indoor Arena" },
  { code: "Stadium (Open)", label: "Stadium (Open)" },
  { code: "Stadium (Closed)", label: "Stadium (Closed)" },
  { code: "Swimming Pool", label: "Swimming Pool" },
...
if (minMonth && maxMonth) {
  const { data: monthRows, error: bmErr } = await supabase
    .from("bill_months")
    .select("bill_month")
    .gte("bill_month", minMonth)
    .lte("bill_month", maxMonth)
    .order("bill_month", { ascending: true });
  if (bmErr) throw bmErr;

  // normalize to YYYY-MM-01 strings and de-dupe
  const uniqueMonthStrings = Array.from(new Set(
    (monthRows ?? []).map(r => (r.bill_month ?? "").slice(0, 7) + "-01")
  )).sort();

  months = uniqueMonthStrings.map(s => ({ bill_month: s }));
}
  ...
  // Build provider → utility → month map
const providersData = React.useMemo(() => {
  ...
  // For now, reuse the global month range you already compute (billMonths)
  // If you later want per-provider month ranges, we can compute those too.
    ...
    <div className="p-6">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-12">
          ...
          <p className="mt-1 text-sm text-slate-600">
            Sorted by bill month (oldest → newest). Missing months are highlighted so gaps are easy to spot.
          </p>
          ...
          {loadingBills ? (
            ...
            <>
              <BillsTable
                ...
                months={electricMonths}
                rowsByMonth={electricByMonth}
                utility="electric"
              />
              <BillsTable
                title="Gas Bills"
                months={gasMonths}
                rowsByMonth={gasByMonth}
                utility="gas"
              />
            </>
          )}
